...
var target="components/ccs1-dataset/"; // 前置準備資料存放 工作資料夾
var colls=[]; // 集訊原碼
var M=0;
var out=[];
var convertfile=function(fn) {
	var txt=fs.readFileSync(fn,"utf8");
	out.push(JSON.stringify(fn+' '+txt));
//	var C=txt.split(/<_ ?id ?=".+?"\/>.*\r\n/); // 以 <_ id=.../> 區隔 集訊
	...
	console.log(out.length,fn,n,'colls from',coCount,'to',coCount+n-1);
...
glob("guanglu/*.xml",function(err,gfiles){
	...
	glob("zonglu/*.xml",function(err,zfiles){
		...
		console.log(target+"files.js",out.length);
		fs.writeFileSync(target+"files.js","module.exports=["+out.join(",\n")+"]","utf8");
	})
});